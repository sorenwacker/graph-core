...
The target builds the renderer, bundles the preload, and runs Playwright against the result. On a developer machine the app's windows stay hidden throughout: Electron has no headless mode, so the helper sets `GRAPH_CORE_HIDE_WINDOWS=1`, which creates every window with `show: false` and background throttling off, so timers and animation frames keep running in the hidden window. Set `CI=1` to watch a run. In CI the pack runs under xvfb instead, with visible windows on a virtual display: a hidden window has no keyboard focus on Linux, and the caret tests fail without it. The pack is a release gate: the release workflow runs it before building artifacts, and it stays out of the per-push CI job to keep CI minutes for the checks that change most often. To run it on Linux for a branch before tagging, dispatch the CI workflow by hand (`gh workflow run ci.yml --ref <branch>`); the `e2e` job runs only on that trigger and uploads Playwright's `test-results/` as an artifact when it fails. Keyboard shortcuts differ by platform: use `MOD` and `DOC_START` from `e2e/helpers.js` rather than literal modifier keys.
