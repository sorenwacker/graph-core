...
class Database {
   ...
   * Reduces N+1 queries when loading sidebar tree
  ...
  /**
   * Export nodes as CSV (flat format)
   * @param {number|null} nodeId - Root node to export, or null for all
   * @param {string|null} workspaceId - Workspace filter
   * @returns {Object} CSV string and headers
   */
   ...
   * @param {number|null} targetParentId - Parent to import under (null for root)
   * @param {string} workspaceId - Target workspace
   * @returns {Object} Import result with new root ID and counts
   */
  importJSON(data, targetParentId = null, workspaceId = 'work') {
    ...
    function importNodeRecursive(nodeData, parentId) {
      ...
      // Create new node (without id, children, created_at, updated_at)
    ...
    if (data.links && Array.isArray(data.links)) {
      for (const link of data.links) {
        ...
        if (newSourceId && newTargetId) {
          try {
            db.linkNodes(newSourceId, newTargetId)
            linksCreated++
          } catch (e) {
  ...
  importCSV(csvData, targetParentId = null, workspaceId = 'work') {
    ...
    for (let i = 1; i < lines.length; i++) {
      ...
      // Determine parent: use mapped ID if available, otherwise targetParentId
