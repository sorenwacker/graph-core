...
/**
 * Composable for node position management.
 * @param {Object} options - Configuration options
 * @param {Function} options.getWorkspace - Function returning current workspace
 * @param {Function} options.getParentId - Function returning current parent ID
 * @param {Function} options.getCy - Function returning cytoscape instance
 * @returns {Object} Position management functions
 */
export function useNodePositions(options = {}) {
  const { getWorkspace, getParentId, getCy } = options

  function getKey() {
    const workspace = getWorkspace ? getWorkspace() : 'work'
    const parentId = getParentId ? getParentId() : null
    return getPositionsKey(workspace, parentId)
  }

  function load() {
    return loadNodePositions(getKey())
  }

  function save() {
    const cy = getCy ? getCy() : null
    saveNodePositions(cy, getKey())
  }

  function findPosition(nodeId, parentId, savedPositions, childIds = []) {
    const cy = getCy ? getCy() : null
    return findSmartPosition(nodeId, parentId, savedPositions, childIds, cy)
  }
